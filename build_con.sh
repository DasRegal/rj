...
FIRMWARE_NAME="robojuice_con_$HASH_COMMIT.hex"
...
$BUILDER -dump-prefs -logger=machine -hardware $HARDWARE -hardware $PACKAGES -tools $IDE_PWD/tools-builder -tools $HARDWARE/tools/avr -tools $PACKAGES -built-in-libraries $IDE_PWD/libraries -libraries $IDE_PWD/libraries -fqbn=CONTROLLINO_Boards:avr:controllino_maxi -vid-pid=0X2341_0X0042 -ide-version=10805 -build-path $PWD/$BUILD_DIR -warnings=none -build-cache $PWD/$CACHE_DIR -prefs=build.warn_data_percentage=75 -prefs=runtime.tools.avrdude.path=$HARDWARE/tools/avr -prefs=runtime.tools.arduinoOTA.path=$HARDWARE/tools/avr -prefs=runtime.tools.avr-gcc.path=$HARDWARE/tools/avr -prefs=compiler.cpp.extra_flags=-DCOMMIT=\"$HASH_COMMIT\" -verbose ./robojuice.ino
